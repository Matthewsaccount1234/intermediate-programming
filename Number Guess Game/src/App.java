...
public class App {
    // Declaring fields for the range of the number, creating the number, and the number of guesses
    ...
    public static int get_new_guess() {
        ...
        if (sc.hasNextInt()) {
            //if input is an integer, it transfers the input to an integer and returns that value
            String next_guess = sc.next();
            int guess_int = Integer.valueOf(next_guess);
            return guess_int;
        } else {
            //if input is not an integer, it recurses the function so that you can guess again with no penalty to the number of guesses
